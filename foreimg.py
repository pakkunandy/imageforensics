...
def noise_inconsistencies(file_path, block_size):
    ...
    for i in range(0,len(cD),block_size):
        for j in range(0,len(cD[0]),block_size):
            ...
            block[(i-1)/block_size+1,(j-1)/block_size+1,:]= temp;
...
def median_noise_inconsistencies(file_path, n_size):
    ...
    plt.subplot(1,2,2),plt.imshow(noise_map),plt.title('Analysis')
    plt.xticks([]), plt.yticks([])
    plt.suptitle('Exposing digital forgeries by using Median-filter noise residue inconsistencies')
    plt.show()
...
def cfa_tamper_detection(file_path):
    ...
    small_cfa_list = np.asarray([[[2, 1],[3, 2]], [[2, 3],[1, 2]],[[3, 2],[2, 1]], [[1, 2],[2, 3]]])
    ...
    for i in range(cfa_list.shape[0]):
        ...
        bin_filter[:,:,0] = np.matlib.repmat(r, img.shape[0]/2, img.shape[1]/2)
        bin_filter[:,:,1] = np.matlib.repmat(g, img.shape[0]/2, img.shape[1]/2)
        bin_filter[:,:,2] = np.matlib.repmat(b, img.shape[0]/2, img.shape[1]/2)
        ...
        proc_im = 1.0*proc_im
        #print(proc_im.shape)
        block_result = np.zeros((proc_im.shape[0]/w1, proc_im.shape[1]/w1, 6)) 
        
        for h in xrange(0, proc_im.shape[0], w1):
            if h + w1 >= proc_im.shape[0]:
                break

            for k in xrange(0, proc_im.shape[1], w1):
        ...
        diffs.append(np.reshape(block_diffs[:,:,1], (1,block_diffs[:,:,1].size)))
...
def bilinInterolation(cfa_im,bin_filter,cfa):
    ...
    #print(mask)
    sum_bin_filter = np.reshape(np.sum(np.sum(bin_filter, axis = 0), axis = 0), (3))
    ...
    for i in range(3):
        ...
        interp_layer = ndimage.correlate(orig_layer, mask[:,:,i], mode='constant')
